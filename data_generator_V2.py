...
# Zentrale Liste aller Maschinen
base_machines = ["M1", "M2", "M3"]
...
def generate_jsp_data(num_jobs):
    ...
    for j in range(1, num_jobs + 1):
        ...
        # Zufällige Permutation der Maschinen für diesen Job
        machine_sequence = random.sample(base_machines, len(base_machines))
        ...
        while len(machine_sequence) < num_operations:
            ...
            machine_sequence.append(random.choice(base_machines))
        
        for op in range(1, num_operations + 1):
            # Verwende die vorbereitete Maschinensequenz
            primary_machine_id = machine_sequence[op-1]
            
            # Für jede Operation eine zufällige Teilmenge der Maschinen auswählen
            # Mindestens die primäre Maschine, maximal alle Maschinen
            num_machine_options = random.randint(1, len(base_machines))
            if num_machine_options == 1:
                machine_ids = [primary_machine_id]
            else:
                # Stelle sicher, dass die primäre Maschine immer enthalten ist
                other_machines = [m for m in base_machines if m != primary_machine_id]
                additional_machines = random.sample(other_machines, min(num_machine_options-1, len(other_machines)))
                machine_ids = [primary_machine_id] + additional_machines
            
            # Realistische Verarbeitungszeiten je nach Maschinentyp und Produktionsschritt
            if primary_machine_id == "M1":
                # Rahmenfertigung dauert typischerweise länger (15-45 Minuten)
                processing_time = random.randint(15, 45)
            elif primary_machine_id == "M2":
            ...
            if op == 1:
                ...
                production_step = PRODUCTION_STEPS[primary_machine_id][0]
                ...
                available_steps = PRODUCTION_STEPS[primary_machine_id]
            ...
            operations.append({
                "id": f"OP{op}",
                "machineIds": machine_ids,  # Mehrere Maschinenoptionen
        ...
        avg_setup_time = sum(SETUP_TIMES[op["machineIds"][0]]["standard"] for op in operations) / len(operations)
